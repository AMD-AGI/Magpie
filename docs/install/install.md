...
For a reproducible Magpie 0.3.0 beta install, pin the release tag:

```bash
pip install "magpie-eval @ git+https://github.com/AMD-AGI/Magpie.git@v0.3.0"
```
...
To install the same optional set from the Magpie 0.3.0 beta release, pin the
same tag:

```bash
pip install "magpie-eval[all] @ git+https://github.com/AMD-AGI/Magpie.git@v0.3.0"
```
...
- **IntelliKit integrations** (human-readable AMD profiling metrics, AMD
  ...
  ```bash
  pip install "magpie-eval[intellikit] @ git+https://github.com/AMD-AGI/Magpie.git@v0.3.0"
  ```
