...
# Magpie Generic vLLM Multimodal Benchmark Script for MI300X
#
# Variant of vllm_mi300x.sh for vision-language (VL) models.  The server
...
#
# All standard env vars from vllm_mi300x.sh still apply:
...
if [[ -n "${BENCHMARK_BASE_URL:-}" ]]; then
  if [[ "$PHASE" != "client" ]]; then
    echo "[vllm_mi300x_mm] BENCHMARK_BASE_URL set; forcing PHASE=client (was $PHASE)"
    PHASE=client
  fi
fi
