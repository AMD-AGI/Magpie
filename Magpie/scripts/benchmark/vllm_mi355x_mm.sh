...
# Magpie Generic vLLM Multimodal Benchmark Script for MI355X
#
# Variant of vllm_mi355x.sh for vision-language (VL) models.  The server
...
#
# All standard env vars from vllm_mi355x.sh still apply:
...
if [[ -n "${BENCHMARK_BASE_URL:-}" ]]; then
  if [[ "$PHASE" != "client" ]]; then
    echo "[vllm_mi355x_mm] BENCHMARK_BASE_URL set; forcing PHASE=client (was $PHASE)"
    PHASE=client
  fi
fi
...
print(f"[vllm_mi355x_mm] result written to {dst}")
PYEOF
fi
set +x
