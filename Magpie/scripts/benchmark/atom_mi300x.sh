...
# Magpie Generic Atom Benchmark Script for MI300X
#
...
# with vLLM, so the bench client uses --backend vllm unchanged.
#
# Remote server (BENCHMARK_BASE_URL): when set, the client phase points
# benchmark_serving at an external Atom/vLLM-compatible HTTP endpoint
# instead of localhost:$PORT, and forces PHASE=client (no local server
# launch, no server-side cleanup, no SERVER_PID monitoring).
...
if [[ -n "${BENCHMARK_BASE_URL:-}" ]]; then
  if [[ "$PHASE" != "client" ]]; then
    echo "[atom_mi300x] BENCHMARK_BASE_URL set; forcing PHASE=client (was $PHASE)"
    PHASE=client
  fi
fi
...
# Atom profiler integration is not yet wired in Magpie. PROFILE=1 is
# accepted but produces no extra args today; revisit when atom exposes
# a torch-profiler flag.
if [[ "${PROFILE:-}" == "1" ]]; then
  echo "[atom_mi300x] PROFILE=1 received but atom profiler wiring is not yet implemented; ignoring."
fi
